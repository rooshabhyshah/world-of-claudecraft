...
### 5.6 Campaign memory — kept deliberately dumb

Two different things get lumped under "memory"; keep them separate.

- **Game state is not memory — it's a database.** HP, positions, conditions, initiative,
  spell slots, inventory, quest flags live in the engine's store (SQLite). The GM reads them
  straight out as exact facts. No embeddings, no decay, no forgetting — and **never
  summarized**.
- **Narrative memory** (what happened, who hates whom, lore invented in play) is the only
  real "memory" problem, and v1 keeps it plain: recent play stays in the context window;
  when it eventually overflows, roll the oldest part into a **running summary**; and keep a
  small **pinned-facts block** (party roster, hard constraints like "Lloyd can't die," key
  reveals) that is always included verbatim and never summarized away. A cheap post-turn
  step appends newly-established facts.

That is the whole v1 design: **state DB + context window + rolling summary + pinned facts.**
Vector search, per-category decay, and "heat" (see the aidm write-up in
`pf2e-reference-projects.md`) are optimizations for when histories get genuinely huge — many
sessions away — **not foundations**. Don't build them first.

### 5.7 Multi-agent personas (generalizes 5.2) — deferred past v1

> **v1: do not run a separate LLM agent per character.** It multiplies cost and latency and
> invites drift/contradiction. A character's "flavor" (reckless barbarian vs cautious
> cleric) is a **system-prompt swap on the single controller**, switched per character — not
> parallel agent processes. Treat the setup below as a later option, only if there's a
> concrete reason to pay for it.

If/when it's worth it, the research-backed guardrails:
...
> **Scope it honestly.** charbuilder rates **build choices** ("is this feat good to *take*"),
> which is **not** the same as "what's the best **action this turn** given the board."
> **v1 = dumb tier badges:** show each legal action's S/A/B/C/D rating where one exists. A
> real "play the optimal move" advisor (positioning, MAP, save-vs-AC, action economy) is a
> separate, much harder system — **deferred**; don't promise it.
...
- **Budget per encounter, not per call** — agent loops use ~4× (single) to ~15× (multi)
  the tokens of plain chat. **Don't run an LLM for every creature's turn** — a 6-enemy fight
  is dozens of model calls (slow and costly). Reserve the model for the GM, narration, and
  *your* characters; run trash mobs on cheap **scripted** tactics (the engine already
  supports scripted controllers). Cap turns; parallelize independent resolutions.
  ([agent-loop latency](https://mlsystems.dev/blog/agent-loop-latency/))
...
- **Character agents** *(deferred — see §5.7; v1 uses one controller with per-character
  prompt flavor)*: structured role/goal/backstory persona + own retrieval memory,
...
### D. Campaign memory — the v1 version is dumb on purpose

Separate **exact state** from **narrative memory** (see §5.6). State (HP, positions,
conditions, inventory, quest flags) is a database read, not memory — exact, never
summarized. Narrative memory in v1 is three plain things:

1. a small **pinned-facts block** always included verbatim (party, hard constraints, key
   reveals) — never summarized away;
2. **recent play in the context window**;
3. a **rolling summary** of the overflow once the window fills, with a cheap post-turn step
   that appends newly-established facts.

The one discipline that matters: **don't trust summaries for facts you need exact — keep the
raw transcript and read from it.** Vector retrieval, per-category decay, and "heat" (the aidm
machinery) are deferred optimizations for very large histories, not v1 architecture.

### E. Recommendations → the rating bridge (UI)

> **v1 = tier badges only** (show each legal action's rating where one exists). The graded
> advisor UX below is **deferred** — see §5.10: a build-rating is not "the best tactical
> action."
